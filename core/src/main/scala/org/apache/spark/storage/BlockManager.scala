...
private[spark] class BlockManager(
  ...
  def getDataSerializeStream(
                           blockId: BlockId,
                           outputStream: OutputStream,
                           values: Iterator[Any],
                           serializer: Serializer = defaultSerializer): SerializationStream = {
    val byteStream = new BufferedOutputStream(outputStream)
    val ser = serializer.newInstance()
    var ss = ser.serializeStream(wrapForCompression(blockId, byteStream));
    try {
      return ss.writeAll(values)
    } catch {
      case e : IOException =>
        ss.close();
        ss = null;
