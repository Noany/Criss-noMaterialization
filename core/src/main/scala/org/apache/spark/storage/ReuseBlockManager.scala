...
private[spark] class ReuseBlockManager() extends ExternalBlockManager with Logging {
  ...
  override def clearBenefit = idToBenefit.clear()
  ...
  override def putValues(blockId: BlockId, values: Iterator[_]): Unit = {
    ...
    val os = if (blockId.name.startsWith("operator")) {
      ...
      try {
        file.getOutStream(WriteType.MUST_CACHE,
  ...
  override def loadBytes(blockId: BlockId): Option[ByteBuffer] = {
    ...
    try {
      ...
      if (!file.isInMemory) {
        ...
        if (changeToMemory) {
          //store file
          //QGWorker.getBenefit(id, SparkEnv.get.conf, qgWorker)
          //assume during execution, benefit wouldn't change
          val benefit = idToBenefit.get(id).getOrElse(QGWorker.getBenefit(id, SparkEnv.get.conf, qgWorker))
          //idToBenefit.put(id, benefit)
          try {
  ...
  override def loadValues(blockId: BlockId): Option[Iterator[_]] = {
    val file = getFile(blockId)
    if (file == null || file.getLocationHosts().size() == 0) {
      return None
    }
    ...
    if (!file.isInMemory) {
      ...
      if (changeToMemory) {
        try {
          client.movePartitionToMemory(id, index, idToBenefit.get(id).getOrElse(QGWorker.getBenefit(id, SparkEnv.get.conf, qgWorker)), file)
        } catch {
          case NonFatal(e) =>
            logWarning(s"Failed to move block $blockId of file ${file.getPath} into Tachyon memory", e)
        }
